...
class LLM:
    ...
    @staticmethod
    def _call(input_text: str, model: str = MODEL) -> Union[requests.Response, str]:
        """
        同步调用LLM API
        
        Args:
            input_text: 输入文本
            model: 模型名称，默认使用配置中的MODEL
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logger.debug(f"LLM响应: {message[:100]}...")
                    return response
                except (KeyError, IndexError):
                    # 处理不同的响应格式
                    message = re.sub(r'<think>[\s\S]*?<\/think>', '', result.get('response', ''), flags=re.DOTALL)
                    logger.debug(f"LLM响应(替代格式): {message[:100]}...")
                    return message.strip()
            else:
                error_msg = f"LLM API请求失败，状态码：{response.status_code}"
                logger.error(error_msg)
                raise RequestException(error_msg)

        except Exception as e:
            error_msg = f"LLM调用发生错误: {str(e)}"
            logger.error(error_msg)
            raise
