...
class ReportGenerator:
    ...
    def _set_chinese_font(self):
	    ...
	    # print("可用字体:", available_fonts) # 可用于调试
	    ...
	    chinese_fonts = [
	        ...
	        # 移除 'DejaVu Sans'，因为它不是真正的中文字体
	    ]
	    
	    # 3. 同时定义常见中文字体文件的路径模式（针对Linux系统）
	    font_path_patterns = [
	        '/usr/share/fonts/truetype/wqy/wqy-microhei.ttc',  # 文泉驿微米黑
	        '/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc',    # 文泉驿正黑
	        '/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc',  # Noto字体
	        '/usr/share/fonts/truetype/noto/NotoSansCJK-Regular.ttc',  # Noto字体另一种路径
	        '~/.fonts/wqy-microhei.ttc',  # 用户目录下的字体
	        '~/.local/share/fonts/wqy-microhei.ttc'  # 另一个可能的用户字体目录
	    ]
	    ...
	    for font_name in chinese_fonts:
	        if font_name in available_fonts:
	            ...
	            print(f"已通过名称设置中文字体为: {font_name}")
	            font_found = True
	            break
	    
	    # 5. 如果通过名称没找到，尝试通过字体文件路径设置
	    if not font_found:
	        for pattern in font_path_patterns:
	            font_path = os.path.expanduser(pattern)  # 扩展~到用户目录
	            if os.path.exists(font_path):
	                try:
	                    ...
	                    plt.rcParams['font.sans-serif'] = [font_name]
	                    plt.rcParams['axes.unicode_minus'] = False
	                    print(f"已通过路径设置中文字体为: {font_name} (路径: {font_path})")
	                    font_found = True
	                    break
	                except Exception as e:
	                    print(f"设置字体路径 {font_path} 时出错: {e}")
	                    continue
	    
	    # 6. 如果仍然没找到，尝试搜索系统中可能的中文字体文件
	    if not font_found:
	        print("尝试搜索系统中可能的中文字体文件...")
	        # 搜索常见的字体目录
	        font_dirs = [
	            '/usr/share/fonts',
	        ...
	        # 常见中文字体文件关键词
	        chinese_keywords = ['chinese', 'chinese', 'cjk', 'sc', 'tc', 'hc', 'noto', 'wqy', 'zenhei', 'microhei', 'simhei', 'msyh']
	        ...
	        for font_dir in font_dirs:
	            ...
	            for root, dirs, files in os.walk(font_dir):
	                for file in files:
	                    if any(keyword in file.lower() for keyword in chinese_keywords) and \
	                       (file.endswith('.ttf') or file.endswith('.ttc') or file.endswith('.otf')):
	                        ...
	                        try:
	                            ...
	                            plt.rcParams['font.sans-serif'] = [font_name]
	                            plt.rcParams['axes.unicode_minus'] = False
	                            print(f"已通过搜索设置中文字体为: {font_name} (路径: {font_path})")
	                            font_found = True
	                            break
	                        except:
	                            continue
	    ...
	    if not font_found:
	        ...
	        # 列出一些可用字体供调试
	        print("部分可用字体:", list(available_fonts.keys())[:10])
	        
	        # 即使没有中文字体，也设置unicode_minus为False防止负号显示问题
	        plt.rcParams['axes.unicode_minus'] = False
	    
	    # 8. 清除matplotlib缓存以确保设置生效
...
"""
    ...
    def generate_report_text(self, df):
        start_time = time.time()
        prompt = GENERATE_REPORT_PROMPT.format(df=df)
        payload = [
            {
                "role": "user",
                "content": prompt
    ...
    def generate_fault_report(self, df, user_request):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with ThreadPoolExecutor(max_workers=2) as executor:
                # 提交两个任务
                report_future = executor.submit(self.generate_report_text, df)
